...
class OutlierDetector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        from .utils import detect_outliers_zscore, detect_outliers_pca
        ...
        if self.method == 'zscore':
            ...
            z_outliers = detect_outliers_zscore(spectral_data, threshold=self.threshold)
            pca_outliers = detect_outliers_pca(spectral_data, threshold=self.threshold)
...
class PCATransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        df_pca = pd.DataFrame(pca_result, index=X.index, columns=component_names)
        
        # Combine with non-spectral columns if specified
        if self.spectral_cols:
            non_spectral_cols = [col for col in X.columns if col not in self.spectral_cols]

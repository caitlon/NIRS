...
def identify_spectral_columns(df: pd.DataFrame) -> Tuple[List[str], List[str]]:
    ...
    non_spectral_cols: List[str] = [
        col for col in df.columns if col not in spectral_cols
    ]
...
def add_sample_identifier(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all(col in df.columns for col in ["SAMPLE NO", "plant", "wetlab ID"]):
        df_with_id["Sample_ID"] = (
            df_with_id["SAMPLE NO"].astype(str)
            + "_"
            + df_with_id["plant"].astype(str)
            + "_"
...
def detect_outliers_zscore(
    ...
    # A sample is an outlier if it's an outlier in at least wavelength_percent
    # of wavelengths

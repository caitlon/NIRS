...
class SavGolConfig(BaseModel):
    ...
    enabled: bool = Field(default=True, description="Whether to apply SG filtering")
    window_length: int = Field(default=15, description="Window length for SG filter")
    polyorder: int = Field(default=2, description="Polynomial order for SG filter")
...
class FeatureSelectionConfig(BaseModel):
    ...
    plot_selection: bool = Field(
        default=False, 
        description="Whether to plot selected features"
    )
    
    # Additional parameters for specific methods
    # Genetic Algorithm
    ga_population_size: int = Field(default=50, description="GA population size")
    ga_n_generations: int = Field(default=20, description="GA number of generations")
    ga_crossover_prob: float = Field(default=0.5, description="GA crossover probability")
    ga_mutation_prob: float = Field(default=0.2, description="GA mutation probability")
    
    # CARS
    cars_n_sampling_runs: int = Field(default=50, description="CARS number of sampling runs")
    cars_exponential_decay: float = Field(default=0.95, description="CARS exponential decay")
    
    # VIP
    vip_n_components: int = Field(default=10, description="VIP number of PLS components")


class ModelConfig(BaseModel):
    """Configuration for model training."""
    model_type: ModelType = Field(
        default=ModelType.PLS, 
    ...
    # PLS
    pls_n_components: int = Field(default=10, description="PLS number of components")
    
    # SVR
    svr_kernel: str = Field(default="rbf", description="SVR kernel")
    svr_C: float = Field(default=1.0, description="SVR regularization parameter")
    svr_epsilon: float = Field(default=0.1, description="SVR epsilon parameter")
    svr_gamma: str = Field(default="scale", description="SVR gamma parameter")
    
    # Random Forest
    rf_n_estimators: int = Field(default=100, description="RF number of estimators")
    rf_max_depth: Optional[int] = Field(default=None, description="RF max depth")
    rf_min_samples_split: int = Field(default=2, description="RF min samples split")
    rf_min_samples_leaf: int = Field(default=1, description="RF min samples leaf")
    
    # XGBoost
    xgb_n_estimators: int = Field(default=100, description="XGB number of estimators")
    xgb_learning_rate: float = Field(default=0.1, description="XGB learning rate")
    ...
    xgb_colsample_bytree: float = Field(default=0.8, description="XGB colsample_bytree")
    
    # LightGBM
    lgbm_n_estimators: int = Field(default=100, description="LGBM number of estimators")
    lgbm_learning_rate: float = Field(default=0.1, description="LGBM learning rate")
...
class DataConfig(BaseModel):
    ...
    remove_outliers: bool = Field(
        default=False,
        description="Whether to detect and remove outliers"
    )

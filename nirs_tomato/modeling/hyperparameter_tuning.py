...
from .regression_models import train_regression_model, evaluate_regression_model
...
def bayesian_hyperparameter_search(
    ...
    if study_name is None:
        study_name = f"{model_type}_optimization_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        params = _get_hyperparameters_for_trial(trial, model_type, random_state)
    ...
    if verbose:
        logger.info(f"Starting Bayesian hyperparameter optimization for {model_type.upper()} model")
        logger.info(f"Running {n_trials} trials with {cv}-fold cross-validation")
...
def _get_hyperparameters_for_trial(
    ...
    if model_type == 'pls':
        ...
        return {
            ...
            'gamma': trial.suggest_categorical('gamma', ['scale', 'auto']) if trial.suggest_categorical('gamma_auto', [True, False]) 
        ...
        return {
            ...
            'max_depth': trial.suggest_int('max_depth', 3, 50) if trial.suggest_categorical('limit_depth', [True, False]) else None,

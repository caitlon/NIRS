...
def setup_mlflow(
    ...
    if experiment is None:
        if artifacts_uri:
            ...
            logger.info(f"Created new experiment '{experiment_name}' with artifacts at {artifacts_uri}")
        else:
...
def log_model(
    ...
    if input_example is None and hasattr(model, 'feature_importances_'):
        # For models that have feature importances, create a random example with correct shape
...
def create_remote_tracking_uri(
    ...
    if storage_type == "s3":
        if endpoint_url:
            tracking_uri = f"{storage_type}://{bucket_name}?endpoint_url={endpoint_url}"
        else:

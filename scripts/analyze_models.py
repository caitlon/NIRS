        ...
        if model_type in ["rf", "xgb"]:
            if "model" in model_data:
                ...
                if hasattr(model, "feature_importances_"):
                    ...
                    feature_names = model_data.get("feature_names", [f"feature_{i}" for i in range(len(importances))])
...
def plot_model_metrics(models_df: pd.DataFrame, output_dir: str):
    ...
    if "model_type" in models_df.columns:
        metrics_by_type = models_df.groupby("model_type").mean(numeric_only=True).reset_index()
...
def main():
    ...
    if "rmse" in models_df.columns:
        ...
        for _, row in models_df_sorted.iterrows():
            ...
            logger.info(f"{row['model_name']:<30} {rmse:<10.4f} {r2:<10.4f} {mae:<10.4f}")

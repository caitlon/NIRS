...
from tests.fixtures.test_data import sample_spectra_data
...
def test_savgol_transformer(sample_spectra_data):
    ...
    for i in range(transformed_data.shape[0]):
        ...
        assert abs(np.sum(row)) < abs(np.sum(sample_spectra_data.iloc[i].values))
...
def test_transformers_with_subset_columns(sample_spectra_data):
    ...
    spectral_cols = [col for col in data_with_extra.columns if col.startswith('wl_')]
    ...
    assert np.array_equal(snv_result['metadata'].values, data_with_extra['metadata'].values)

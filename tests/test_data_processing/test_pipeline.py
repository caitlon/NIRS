...
from nirs_tomato.data_processing.transformers import SNVTransformer, SavGolTransformer
...
def test_pipeline_with_feature_selection(sample_spectra_data, sample_target_data):
    """Test pipeline with feature selection component."""
    # Create pipeline with preprocessing and feature selection
    pipeline = Pipeline([
        ('snv', SNVTransformer()),
        ('savgol', SavGolTransformer(window_length=11, polyorder=2, deriv=1)),
        ('feature_selector', PLSVIPSelector(n_components=3, n_features_to_select=10))
    ])

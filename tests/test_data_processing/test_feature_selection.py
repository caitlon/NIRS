...
from tests.fixtures.test_data import sample_spectra_data, sample_target_data
...
def test_pls_vip_selector():
    ...
    selector = PLSVIPSelector(n_components=5, n_features_to_select=n_features_to_select)
    ...
    important_features_selected = sum(1 for i in range(10) if f"feature_{i}" in X_selected.columns)
    assert important_features_selected >= 3  # At least 3 of the important features should be selected


def test_pls_vip_selector_with_real_data(sample_spectra_data, sample_target_data):
    ...
    # Basic checks
    assert X_selected.shape == (sample_spectra_data.shape[0], n_features_to_select)
    assert hasattr(selector, 'vip_scores_')

...
class ExperimentManager:
    ...
    def run_from_config(self, config_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Error running experiment from {config_path}: {str(e)}")
            raise
    
    def run_from_config_dir(self, config_dir: str) -> Dict[str, Dict[str, Any]]:
        """
        ...
        if not config_files:
            self.logger.warning(f"No YAML configuration files found in {config_dir}")
            return results
        ...
        for config_file in config_files:
            try:
                ...
                self.logger.error(f"Error running experiment from {config_file}: {str(e)}")
                # Continue with next experiment instead of stopping
                results[experiment_name] = {"error": str(e)}
        
        return results
    
    def run_from_config_object(self, config: ExperimentConfig) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            processed_data = self._apply_feature_selection(config, processed_data)
        ...
        if config.feature_selection.method != "none":
            ...
            if config.feature_selection.plot_selection and hasattr(feature_selector, 'plot_selected_wavelengths'):
                ...
                if hasattr(feature_selector, 'selected_features_indices_'):
                    ...
                    self.logger.info(f"Saved selected features to {selected_features_path}")
    ...
    def _train_and_evaluate(
        ...
        if config.model.tune_hyperparams:
            ...
            if config.mlflow.enabled:
                log_parameters({f"tuned_{k}": v for k, v in best_params.items()})
        else:
    ...
    def _save_results(self, config: ExperimentConfig, results: Dict[str, Any]) -> None:
        """Save experiment results."""
        # Save predictions to CSV
        predictions_path = os.path.join(config.results_dir, f"{config.name}_predictions.csv")
        pd.DataFrame({
        ...
        metrics_path = os.path.join(config.results_dir, f"{config.name}_metrics.txt")

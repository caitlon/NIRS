...
def run_experiment(
    ...
    if use_mlflow:
        ...
        run_name = (
            f"{experiment_name_str}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    if process.returncode == 0:
        ...
        model_files = sorted(
            [
                ...
                if f.startswith(f"{model_type}_{target_column}_")
            ],
        ...
        if model_files:
                    ...
                    if isinstance(model_data, dict):
                        if "metrics" in model_data:
                            ...
                            metrics = {
                                ...
                                if k
                                in ["rmse", "r2", "mae", "explained_variance"]
                            }
                        ...
                        if (
                            "model" in model_data
                            and "feature_names" in model_data
                        ):
                            ...
                            input_example = pd.DataFrame(
                                np.random.random((1, len(feature_names))),
                            ...
                            if hasattr(model, "feature_importances_"):
                                ...
                                plt.yticks(
                                    range(len(sorted_idx)), plot_feature_names
                                )
                                plt.title(
                                ...
                                log_figure(
                                    fig,
                                    f"artifacts/feature_importance_{model_type}.png",
                                )
            ...
            logger.warning(
                f"No model file found for experiment {experiment_name_str}"
            )
            if use_mlflow:
                end_run()
            return None
    ...
    try:
        ...
        if isinstance(model_data, dict) and "metrics" in model_data:
            ...
            if metrics:
                ...
                logger.warning(
                    f"Could not extract metrics from model file: {model_file}"
                )
                # Return empty values
...
def main():
    ...
    if args.use_mlflow:
        parent_run_name = (
            f"experiment_set_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        start_run(
    ...
    for exp in experiments:
        ...
        if exp.get("feature_selection_method"):
            exp_name += (
                f"_{exp['feature_selection_method']}{exp['n_features']}"
            )
        ...
        if model_file:
            ...
            result = {
                ...
                "use_savgol": exp["use_savgol"],
                "tune_hyperparams": exp.get("tune_hyperparams", False),
                "feature_selection_method": exp.get(
            ...
            logger.warning(
                f"Experiment {exp_name} failed to produce a model file"
            )
    ...
    if not results_df.empty:
        ...
        results_file = os.path.join(
            args.results_dir, f"experiment_results_{timestamp}.csv"
        )
                ...
                log_parameters(
                    {
                        ...
                        "best_tune_hyperparams": bool(
                            best_model["tune_hyperparams"]
                        ),
                        "best_feature_selection": best_model[
                            "feature_selection_method"
                        ],

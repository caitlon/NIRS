...
def create_config_template(
    ...
    if not description:
        transform_name = "SNV" if transform == "snv" else "MSC" if transform == "msc" else "no"
        fs_name = "without feature selection" if feature_selection == "none" else f"with {feature_selection.upper()} feature selection"
    ...
    data_config = DataConfig(
        ...
        savgol=SavGolConfig(
            ...
            polyorder=2,
            deriv=0
        ) if transform != "none" else SavGolConfig(enabled=False)
    )
...
def main():
    ...
    parser.add_argument(
        "--data_path", type=str, default="data/raw/Tomato_Viavi_Brix_model_pulp.csv",
        help="Path to input data CSV file"
    )
    parser.add_argument(
        "--target_column", type=str, default="Brix",
        help="Name of target column in data"
    )
    parser.add_argument(
        "--transform", type=str, default="snv", choices=["snv", "msc", "none"],
        help="Spectral transformation type"
    )
    parser.add_argument(
        "--model", type=str, default="pls", choices=["pls", "svr", "rf", "xgb", "lgbm"],
        help="Type of regression model"
    )
    parser.add_argument(
        "--feature_selection", type=str, default="none", choices=["none", "ga", "cars", "vip"],
